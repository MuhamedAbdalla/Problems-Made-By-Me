...
/**
 * Modified blocks are wrapped inside of //+++ ... //---.
 */
...
//+++
GoogieSpell.escapeUrl = function(val) {
    return val.replace(/%/g, "%25")
            // Reserved characters:
            .replace(/\$/g, "%24").replace(/&/g, "%26").replace(/\+/g, "%2B").replace(/,/g, "%2C").replace(/\//g, "%2F")
            .replace(/:/g, "%3A").replace(/;/g, "%3B").replace(/=/g, "%3D").replace(/\?/g, "%3F").replace(/@/g, "%40")
            // Unsafe characters:
            .replace(/ /g, "%20").replace(/"/g, "%22").replace(/</g, "%3C").replace(/>/g, "%3E").replace(/#/g, "%23")
            .replace(/\{/g, "%7B").replace(/\}/g, "%7D").replace(/\|/g, "%7C").replace(/\\/g, "%5C").replace(/\^/g, "%5E")
            .replace(/~/g, "%7E").replace(/\[/g, "%5B").replace(/\]/g, "%5D").replace(/`/g, "%60");
}
//---
...
GoogieSpell.createXMLReq = function (text) {
    return 'requestBody=<?xml version="1.0" encoding="utf-8" ?><spellrequest textalreadyclipped="0" ignoredups="0" ignoredigits="1" ignoreallcaps="1"><text>' + text + '</text></spellrequest>';
}
//---
...
GoogieSpell.prototype.spellCheck = function(ignore) {
    ...
    var req_text = GoogieSpell.escapeUrl(this.orginal_text);
    //---
